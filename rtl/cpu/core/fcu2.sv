...
// Description  : Instruction consume and BTB redirect generation.
//                Receives one response per accepted I-Cache request and returns
//                the consume event that releases the cache lookup slot.
//
...
// Changes v2.0 : replace CWF-specific state with explicit response consume.
// -----------------------------------------------------------------------------
...
(
    ...
    //IF2 fire: aligned metadata + cache response accepted into IF2/ID
    assign if2_fire       = if2_valid && cache_valid && !stall;
    assign icache_consume = if2_fire;

    //output
    assign instr_o              = instr_i;

    //advance fetch stream when IF2 sees a new live response
    assign cache_advance        = if2_fire;
    ...
    assign if2_id_flush         = ex_mispredict | ((!if2_valid || !cache_valid) && !stall);
